...
const CustomizationManager = {
  ...
  handleBackgroundUpload(e) {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onload = (event) => {
        this.createDraggableImage(event.target.result);
      };
      reader.readAsDataURL(file);
    }
  },

  createDraggableImage(imageSrc) {
    ...
    const resizeBtn = this.createControlButton('↔️', 'Resize');
    const deleteBtn = this.createControlButton('🗑️', 'Delete');
  ...
  makeDraggable(element) {
    ...
    element.addEventListener('mousedown', (e) => {
      if (window.isLayoutLocked) return;
      if (e.target.closest('.image-controls button')) return;
      isDragging = true;
    ...
    document.addEventListener('mouseup', () => {
      if (isDragging) {
        isDragging = false;
        element.style.cursor = window.isLayoutLocked ? 'default' : 'move';
      }
    });
  },
